...
# This file is machine-local and provides defaults for the sync checklist.
SYNC_GENERIC=$generic
SYNC_CODEX=$codex
SYNC_CLAUDE=$claude
...
echo "Choose the default groups for the sync checklist."
migrate_legacy_state
...
sync_generic="$(prompt_yes_no "Sync generic/shared setup on this machine?" "$(existing_yes_no SYNC_GENERIC y)")"
...
sync_omp="$(prompt_yes_no "Sync OMP setup on this machine?" "$(existing_yes_no SYNC_OMP y)")"
...
write_flags "$sync_generic" "$sync_codex" "$sync_claude" "$sync_opencode" "$sync_omp" "$install_shell_commands"
